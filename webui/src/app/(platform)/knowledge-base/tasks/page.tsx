      ...
      <div className="flex min-w-0 flex-1 flex-col gap-4">
        ...
        <div className="rounded-2xl border border-shell-border bg-shell-panel p-4 shadow-sm">
          ...
          <div className="mt-3 flex items-center justify-between">
            <p className="text-[12px] text-shell-muted">⌘ Enter 提交 · Agent 将自动检索知识库</p>
            <button
              type="button"
        ...
        {detail ? (
          ...
          <div className="flex flex-1 flex-col items-center justify-center rounded-2xl border border-dashed border-shell-border bg-shell-panel/50 p-10 text-center">
            ...
            <p className="mt-4 text-[15px] font-medium text-shell-text">提交一个任务</p>
            <p className="mt-1 max-w-xs text-[13px] text-shell-muted">
              Agent 会主动搜索知识库、逐步推理，最终给出结构化答案
            </p>
            <div className="mt-6 space-y-2 text-left">
              {[
                '整理中康尚德所有应收款信息和风险',
                '列出知识库中涉及税务合规的规则',
                '帮我分析企业所得税申报调整流程的关键步骤',
              ].map((ex) => (
                <button
                  key={ex}
                  type="button"
                  onClick={() => setInput(ex)}
                  className="block w-full rounded-lg border border-shell-border bg-shell-bg px-3 py-2 text-left text-[12px] text-shell-muted transition-colors hover:border-brand-primary/40 hover:text-brand-primary"
                >
                  {ex}
                </button>
              ))}
