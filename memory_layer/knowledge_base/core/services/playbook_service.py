...
from memory_layer.knowledge_base.core.services.context_builder import _load_playbook
...
def playbook_override_path(org_id: str):
    return config.STORAGE_ROOT / "orgs" / org_id / "playbook.md"
...
def preview_playbook(org_id: str, user_id: str, content: str | None = None) -> dict:
    ...
    from memory_layer.knowledge_base.core.services.context_builder import build_pinned_block

    kwargs: dict = {}
    if content is not None:
        kwargs["playbook_text"] = content.strip()
    block, pinned = build_pinned_block(org_id, user_id, **kwargs)
    cfg = load("recall").get("pinned") or {}
