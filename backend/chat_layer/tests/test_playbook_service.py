...
def test_save_playbook_clears_pinned_sessions(tmp_path):
    org = "org-pb"
    override = tmp_path / org / "playbook.md"

    with patch("chat_layer.core.services.playbook_service.config") as cfg:
        cfg.ORGS_ROOT = tmp_path
        with patch("chat_layer.core.services.playbook_service._chat_registry") as reg:
            reg.clear_pinned_context_for_org.return_value = 3
            with patch(
                "chat_layer.core.services.playbook_service._load_playbook",
