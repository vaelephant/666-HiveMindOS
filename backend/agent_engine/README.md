...
与 `knowledge_base`（知识沉淀：Wiki / 智慧 / 候选池）解耦；执行时通过 Tool 调用 KB 能力。
...
Prompt 模板仍在 `knowledge_base/prompts/prompts.yaml`（`agents.planning_*` / `agents.final_reflect` 等）。

## API 入口

`server/routers/tasks.py` → `agent_engine.services.task_service`
