...
def test_save_playbook_clears_pinned_sessions(tmp_path):
    ...
    with patch("knowledge_base.core.services.playbook_service.config") as cfg:
        cfg.STORAGE_ROOT = tmp_path
        with patch(
            "knowledge_base.core.services.playbook_service._chat_registry"
        ) as reg:
            reg.clear_pinned_context_for_org.return_value = 3
            with patch(
                "knowledge_base.core.services.playbook_service._load_playbook",
