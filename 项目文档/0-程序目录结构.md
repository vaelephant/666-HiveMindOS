...
| **知识沉淀层** | `backend/knowledge_base/` | Wiki / 候选池 / Chat / 入库编译 / Playbook 等业务逻辑 | ✅ 在用 |
| **记忆子系统** | `backend/memory_layer/` | L1/L2 智慧、向量召回、进化流水线（**目标位**，逐步从 KB 抽离） | 🔜 抽离中 |
...
│   │   ├── models/ registry/ tools/ domain/ skills/
│   │   └── tests/
│   │
│   ├── knowledge_base/                 # 知识沉淀（业务逻辑，无 HTTP）
│   │   ├── config.py                   # 路径、DB、Qdrant、模型 env
│   │   ├── settings/ prompts/
│   │   ├── core/
│   │   │   ├── pipelines/              # LLM 工序：ingest/query/chat/lint/记忆提炼（非自主 agent）
│   │   │   ├── compiler/              # 实体 / 流程提取 → Wiki
│   │   │   ├── wiki/ graph/ vector/ registry/ services/
│   │   │   ├── tools/kb_toolkit.py
│   │   │   └── domain/ parsers/ db/
│   │   ├── models/ sdk/ storage/ tests/
│   │
│   ├── memory_layer/                   # 记忆子系统（仅 memory，待从 KB 抽离）
│   ├── integrations/                   # 第三方通道（gateway、wechat_work、tests）
│   ├── model_layer/                    # 多 Provider LLM（settings、providers、registry、client）
...
| Wiki / 记忆分类 | `backend/knowledge_base/settings/taxonomy.yaml` | — |
| Chat 工具 schema | `backend/knowledge_base/settings/tools.yaml` | — |
| LLM Prompt 模板 | `backend/knowledge_base/prompts/prompts.yaml` | 任务与 KB 共用，按 `agents.*` 分组 |
| 模型 profile 注册表 | `backend/model_layer/settings/models.yaml` | ~~knowledge_base/config.py 硬编码~~ |
...
| 2026-06-13 | 工作流 cron：schedule_enabled、进程内调度、run_scheduled_workflows.py |
