...
| **模型层** | `model_layer/` | 多 Provider LLM / Embedding（profile 注册表） | ✅ 在用 |
...
│   ├── settings/
│   │   ├── loader.py
│   │   └── models.yaml                 # profile → provider + model + dim
│   ├── providers/
│   │   ├── base.py                     # ChatProvider / EmbedProvider
│   │   ├── openai_provider.py          # chat + stream + tools + embed
│   │   └── anthropic_provider.py       # chat + stream + tools
│   ├── registry.py                     # resolve profile、启动校验
│   ├── client.py                       # llm.complete / embed 门面
│   └── tests/
│
├── webui/                                # Next.js 前端
│   ├── prisma/
│   │   └── schema.prisma                 # 用户表（users，与 PG 共用）
│   └── src/
│       ├── auth.ts                       # NextAuth v5（Credentials + JWT）
│       ├── middleware.ts                 # 登录守卫 + KB API org 校验
...
│       ├── app/auth/                     # 登录 / 注册 / 推广页
│       ├── app/api/auth/                 # NextAuth handler + register
│       ├── app/api/kb/[orgId]/           # BFF 代理 → FastAPI :8006
│       ├── components/auth/              # SessionProvider、OrgProvider、UserMenu
│       ├── components/knowledge-base/      # agent-tasks-view、agent-task-detail…
│       └── lib/                          # kb-api、users、prisma、kb-types
│
├── scripts/                              # 运维与开发辅助
...
| 模型 profile 注册表 | `model_layer/settings/models.yaml` | ~~memory_layer/.../config.py 硬编码~~ |
