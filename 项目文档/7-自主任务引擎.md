...
自主任务引擎独立为顶层包 **`agent_engine/`**（与 `knowledge_base` 知识沉淀层、`memory_layer` 记忆子系统并列）：

| 路径 | 职责 |
|------|------|
| `agent_engine/settings/` | 委员会、工具白名单、gate、rubrics |
| `agent_engine/agents/` | Planner、PlanningCommittee、Reflect、Replan |
| `agent_engine/execution/` | Orchestrator、Executor |
| `agent_engine/services/` | `task_service`、经验召回 |
| `knowledge_base/` | Wiki / 智慧 / 候选池 / Chat API（沉淀层） |

## 配置

| 文件 | 说明 |
|------|------|
| `agent_engine/settings/task_tools.yaml` | Planner 可见 action |
| `agent_engine/settings/task_gates.yaml` | 重试上限、compile 人工门 |
| `agent_engine/settings/rubrics/*.yaml` | 任务评分标准 |
| `agent_engine/settings/planning_committee.yaml` | 规划委员会角色 |
| `TAVILY_API_KEY`（可选） | 高质量网络搜索 |
| `EXPERIENCE_COLLECTION` | 经验向量集合名，默认 `hivemind_experiences` |
| `action_gates.wechat_work_send` | 默认 `human`，企微出站需人工批准 |
| `save_deliverable` | 将任务交付物写入 Wiki `deliverables/` |
...
| 审计日志 | [11-审计日志.md](./11-审计日志.md) |
| 自主任务页 | webui → `/tasks/agent`（任务中心） |
| 定时运维页 | webui → `/tasks/ops` |
| Chat 升级入口 | `/hivemind-chat` → 智能推荐或「从此对话创建任务」 |
| 规划委员会 | `agent_engine/settings/planning_committee.yaml`；`chat_upgrade` / `task_center` 触发 |
| 配置说明 | [8-规划委员会配置.md](./8-规划委员会配置.md) |
| 模块 README | `agent_engine/README.md` |
...
| 2026-06-13 | save_deliverable、wechat 人工门、审计钩子 |
